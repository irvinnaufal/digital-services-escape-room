...
const nextButton = document.getElementById("finish-level2-btn");
...
function checkCompletion() {
  ...
  const operationsTasks = Array.from(operationsZone.children).map(
    (task) => task.id
  );
  if (
    !operationsTasks.includes("task1") ||
    !operationsTasks.includes("task2") ||
    !operationsTasks.includes("task3")
  ) {
    correctPlacement = false;
  }

  // IT Solutions validation
  const solutionsTasks = Array.from(solutionsZone.children).map(
    (task) => task.id
  );
  if (
    !solutionsTasks.includes("task4") ||
    !solutionsTasks.includes("task5") ||
    !solutionsTasks.includes("task6")
  ) {
    correctPlacement = false;
  }
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // If Level 1 is not completed, redirect to Level 1
    if (!level1Complete) {
        alert("You must complete Level 1 first!");
        window.location.href = 'level1.html'; // Redirect to Level 1
    }

    // Code for Level 2 goes here
    // When the player finishes Level 2:
    const finishButton = document.getElementById('finish-level2-btn');
    finishButton.addEventListener('click', function () {
        // Mark Level 2 as completed in localStorage
